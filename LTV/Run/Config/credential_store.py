...
# COMPLETO - VERSIÓN MEJORADA CON DUAL-WRITE
# ============================================================================
"""
...
class CredentialStore:
    ...
    # Referencia al CSV legacy (para dual-write)
    CSV_PATH = Path(__file__).parent / "data_xlsx" / "credentials_vault.csv"
    ...
    def _get_default_structure(cls) -> Dict:
        ...
        return {
            ...
            "db": {"user": "", "password": ""},
            "countries": {
                "GT": {"ssh": "", "host": "127.0.0.1:3336", "database": "db_pacifiko"},
                "CR": {"ssh": "", "host": "127.0.0.1:3337", "database": "CRProdDb"}
            },
            "users": {}
        }
    ...
    def _load_csv(cls) -> Optional[Dict]:
        """Carga credenciales desde CSV legacy."""
    ...
    def _save_csv_legacy(cls, data: Dict):
        """Guarda credenciales en CSV legacy (dual-write)."""
        if not cls.CSV_PATH.parent.exists():
        ...
        try:
            ...
            print(f"   📝 Dual-write: CSV actualizado")
            return True
        except Exception as e:
    ...
    def migrate_from_csv(cls) -> bool:
        ...
        if csv_data and csv_data.get('db', {}).get('user'):
            ...
            if cls.CSV_PATH.exists():
                timestamp = datetime.now().strftime('%Y%m%d_%H%M%S')
                backup_csv = cls.CSV_PATH.parent / f"credentials_vault_backup_{timestamp}.csv"
                shutil.copy2(cls.CSV_PATH, backup_csv)
    ...
    def clear(cls):
        ...
        print("   CSV legacy conservado como backup")
