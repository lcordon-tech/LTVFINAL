...
# DEPRECATED - VERSIÓN LEGACY PARA COMPATIBILIDAD
# ============================================================================
"""
...
⚠️ DEPRECATED: Este archivo se mantiene por compatibilidad.
   ...
   Las escrituras se redirigen a CredentialStore automáticamente.
"""
...
class VaultManager:
    ...
    Usar CredentialStore para nuevo código.
    """
    ...
    def get_first_credentials(self, country: Optional[str] = None) -> Optional[Dict]:
        ...
        if self._cred_store:
            ...
            if db_creds and db_creds.get('user'):
                ...
                ssh_cmd = self._cred_store.get_ssh_command(country) if country else ""
                host = self._cred_store.get_host(country) if country else "localhost"
                database = self._cred_store.get_database(country) if country else ""
                
                return {
                    'country': country or 'GT',
        ...
        with open(self.vault_path, 'r', encoding='utf-8') as f:
                ...
                for row in rows:
                    if row.get('country') == country:
                        return {
                            ...
                            'db_name': row.get('db_name', '') or f"db_{country.lower()}",
                        }
                return None
            ...
            if rows:
                ...
                return {
                    ...
                    'ssh_cmd': row.get('ssh_cmd', ''),
                    'host': row.get('host', 'localhost'),
                    'db_name': row.get('db_name', '') or 'db_pacifiko',
                }
