...
class CohortSupuestosManager:
    ...
    def _detect_mode(self) -> str:
        """Detecta hojas que terminan con el código del país."""
        try:
            ...
            for sheet in sheet_names:
                if sheet.endswith(self.country_code):
                    print(f"📋 Modo multi-país detectado: usando hojas que terminan en '{self.country_code}'")
                    return 'country_suffix'
            
            # Fallback a modo legacy
            print(f"📋 Modo legacy detectado: usando hojas estándar (1P, 3P, etc.)")
            return 'legacy'
