...
class TimeGranularityAdapter:
    ...
    def __init__(self, granularity: str = 'quarterly'):
        """
    ...
    def _parse_quarter(self, cohort: str) -> Tuple[int, int]:
        ...
        Returns:
            (year_offset, quarter_number)
            year_offset: 0 = 2021, 1 = 2022, etc.
        """
        cohort_clean = cohort.upper().replace('Q', '')
        
        # Manejar cohortes negativas (Q-3, Q-2, Q-1, Q0)
        if cohort_clean.startswith('-'):
            num = int(cohort_clean)
            return (num, 1)  # Simplificado para cohortes pre-2021
        
        num = int(cohort_clean)
        # Q1 = 2021-Q1 (año 0, trimestre 1)
        # Q5 = 2022-Q1 (año 1, trimestre 1)
        year_offset = (num - 1) // 4
        quarter_num = ((num - 1) % 4) + 1
        return (year_offset, quarter_num)
    ...
    def _quarter_to_month_cohorts(self, cohort: str, retention_value: float, cogs_value: float) -> Dict[str, Dict]:
        ...
        """
        year_offset, quarter = self._parse_quarter(cohort)
        base_year = 2021 + year_offset
        ...
        result = {}
        for month in months:
            month_cohort = f"{base_year}-{month:02d}"
            result[month_cohort] = {
    ...
    def _quarter_to_week_cohorts(self, cohort: str, retention_value: float, cogs_value: float) -> Dict[str, Dict]:
        ...
        for i in range(weeks_per_quarter):
            ...
            week_cohort = f"{base_year}-W{week_num:02d}"
            result[week_cohort] = {
    ...
    def _quarter_to_semiannual_cohorts(self, cohort: str, retention_value: float, cogs_value: float, 
        ...
        if quarter in [1, 2]:
            half = 1
            half_cohort = f"{base_year}-H1"
            # Sumar retention de Q1 y Q2
            # Nota: esto requiere pasar el siguiente trimestre
            ...
            half = 2
            half_cohort = f"{base_year}-H2"
            total_retention = retention_value * 2
        
        cogs_half = cogs_value  # COGS se mantiene
        
        return {half_cohort: {'retention': total_retention, 'cogs': cogs_half}}
    ...
    def _quarter_to_yearly_cohorts(self, cohorts_retention: Dict[str, float], 
        ...
        for cohort, retention in cohorts_retention.items():
            year_offset, _ = self._parse_quarter(cohort)
            year = 2021 + year_offset
    ...
    def _transform_semiannual(self, retention_map: Dict[str, float], 
        ...
        sorted_cohorts = sorted([c for c in retention_map.keys() if c.startswith('Q') and len(c) > 1],
                                key=lambda x: int(x[1:]) if x[1:].lstrip('-').isdigit() else 0)
    ...
    def _transform_yearly(self, retention_map: Dict[str, float], 
        ...
        return self._quarter_to_yearly_cohorts(retention_map, cogs_map)
    ...
    def transform(self, retention_map: Dict[str, float], 
        ...
        print(f"🔄 Transformación aplicada: {self.granularity}")
