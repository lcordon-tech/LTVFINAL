...
def run_pipeline():
    ...
    try:
        ...
        q_engine = QueryEngine(
            ...
            host=HOST, 
            db=DB,
            country_code=country_code  # ← NUEVO: pasa el país para seleccionar query
        )
        
        loader = DataLoader(query_engine=q_engine, base_dir=OUTPUT_DIR)
        validator = DataValidator()
        merger = DataMerger()
        cohort_handler = CohortBuilder(granularidad=granularidad)
        retention_handler = RetentionApplier(granularidad=granularidad)
        applier = AssumptionApplier()
        
        # ⭐ MetricsCalculator recibe country_context y fx_engine
        ...
        # Filtrar cohortes por año de inicio del país (si es necesario)
        if country_context.cohort_start_year > 2021:
            def get_cohort_year(cohort):
                try:
            ...
            df_orders_cohorts['_cohort_year'] = df_orders_cohorts['cohort'].apply(get_cohort_year)
